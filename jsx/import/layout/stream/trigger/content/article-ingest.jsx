...
class ArticleIngest extends Component {
    ...
    constructor(props) {
        ...
        if (
            ...
            var listing_graphic_title = 'Article Ingest';
        }

        if (
            'source_name' in this.props
            && checkValidString(this.props.source_name)
        ) {
            var source_name = this.props.source_name;
        } else {
            var source_name = 'our configured sources';
        }
        ...
        // An hourly stream drew a minute axis.
        //
        if (
    ...
    render() {
        ...
        const summary_below = (
            <div className='summary-item'>{`
                Our system captures articles, journals, filings, and alerts from
                ${this.state.source_name}. To get exact list of partitions, please review the ingest
                performance via Archive (or datalake). These categories are aggregated and batched
                ${this.state.ingest_interval}, then stored to our datalake and stream, ready to be
                consumed by stream application(s).
            `}</div>
        );

        const summary_integration = (
            <div>{`
                You select either the title or summary and perform NLP analysis. Alerts are aggregated
                and batched into the datalake and stream ${this.state.ingest_interval} every day. Our
                system performs tumbling-window operation on the data stream, with your selected NLP
                process(es), when a set criteria is detected you get notified. You can choose and
                customize workflows using basic triggers, trigger aggregate, and machine-learning
                integration. More advanced workflows will become available early-2025 (stay tuned).
            `}</div>
        );
